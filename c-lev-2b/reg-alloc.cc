...
{ 
	if ((reg_use == gp_data) && (lra_symbol_list.empty()) && !(used_for_expr_result))  //updated must not be in use for expression result 
		return true;
	else 
		return false;
}
...
void Lra_Outcome::optimize_lra(Lra_Scenario lcase, Ast * destination_memory, Ast * source_memory)
{
	...
	case mc_2m:
		...
		{
			reg_type = float_num;
			if(source_memory->get_data_type() == int_data_type) reg_type = int_num;
			
			result_register = machine_dscr_object.get_new_register(reg_type);
...
void Machine_Description::initialize_instruction_table()
{
	...
	spim_instruction_table[cast_to_float] = new Instruction_Descriptor(cast_to_float, "mtc1", "mtc1", "",  i_r_op_o1, a_op_r_o1);
	spim_instruction_table[cast_to_int] = new Instruction_Descriptor(cast_to_int, "mfc1", "mfc1", "",  i_r_op_o1, a_op_r_o1);
	...
	spim_instruction_table[op_add] = new Instruction_Descriptor(op_add, "add", "add", "", i_r_o1_op_o2, a_op_r_o1_o2);
	spim_instruction_table[op_sub] = new Instruction_Descriptor(op_sub, "sub", "sub", "", i_r_o1_op_o2, a_op_r_o1_o2);
	spim_instruction_table[op_mul] = new Instruction_Descriptor(op_mul, "mul", "mul", "", i_r_o1_op_o2, a_op_r_o1_o2);
	spim_instruction_table[op_div] = new Instruction_Descriptor(op_div, "div", "div", "", i_r_o1_op_o2, a_op_r_o1_o2);
	spim_instruction_table[op_uminus] = new Instruction_Descriptor(op_uminus, "uminus", "neg", "",  i_r_op_o1, a_op_r_o1);

	spim_instruction_table[op_add_d] = new Instruction_Descriptor(op_add_d, "add.d", "add.d", "", i_r_o1_op_o2, a_op_r_o1_o2);
	spim_instruction_table[op_sub_d] = new Instruction_Descriptor(op_sub_d, "sub.d", "sub.d", "", i_r_o1_op_o2, a_op_r_o1_o2);
	spim_instruction_table[op_mul_d] = new Instruction_Descriptor(op_mul_d, "mul.d", "mul.d", "", i_r_o1_op_o2, a_op_r_o1_o2);
	spim_instruction_table[op_div_d] = new Instruction_Descriptor(op_div_d, "div.d", "div.d", "", i_r_o1_op_o2, a_op_r_o1_o2);
	spim_instruction_table[op_uminus_d] = new Instruction_Descriptor(op_uminus_d, "uminus.d", "neg.d", "",  i_r_op_o1, a_op_r_o1);
...
Register_Descriptor * Machine_Description::get_new_register(Register_Val_Type type)
{
	...
	for (i = spim_register_table.begin(); i != spim_register_table.end(); i++)
	{
		...
		if (reg_desc->is_free() && reg_desc->get_val_type()==type){
			reg_desc->set_used_for_expr_result(); //thus this register is now busy unless freed using reset()
			return reg_desc;
		}
	}
