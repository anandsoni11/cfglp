...
Code_For_Ast & Relational_Expr_Ast::compile_and_optimize_ast(Lra_Outcome & lra)
{
	...
	if(typeid(*lhs) == typeid(Number_Ast<int>) || typeid(*lhs) == typeid(Number_Ast<float>)){
...
Code_For_Ast & Name_Ast::compile_and_optimize_ast(Lra_Outcome & lra)
{
	...
	if (load_needed)
	{
		...
		load_stmt = new Move_IC_Stmt(tgt_operation, opd, register_opd);
...
Code_For_Ast & If_Ast::compile_and_optimize_ast(Lra_Outcome & lra)
{
	...
	if(typeid(*condition) == typeid(Number_Ast<int>) || typeid(*condition) == typeid(Number_Ast<float>)){
...
Code_For_Ast & Type_Cast_Ast::compile_and_optimize_ast(Lra_Outcome & lra)
{
	...
	Lra_Outcome lra_expr;

	if(typeid(*expression) == typeid(Number_Ast<int>) || typeid(*expression) == typeid(Number_Ast<float>)){
		lra_expr.optimize_lra(c_2r, NULL, expression);
	}
	else if(typeid(*expression) == typeid(Name_Ast)){
		lra_expr.optimize_lra(m_2r, NULL, expression);
	}
	
	Code_For_Ast & expression_stmt = expression->compile_and_optimize_ast(lra_expr);
