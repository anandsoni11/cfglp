...
{
	//if(op == imm_load) cout << "sstore" << endl;
...
void Compute_IC_Stmt::print_icode(ostream & file_buffer)
{
	...
	//cout << "icode format " << ic_format <<" " << i_r_o1_op_o2 <<" " << i_r_op_o1 << endl;
...
Control_Flow_IC_Stmt::Control_Flow_IC_Stmt(Tgt_Op inst_op, Ics_Opd * o1, Ics_Opd * o2, Ics_Opd * o3)
{
	CHECK_INVARIANT((machine_dscr_object.spim_instruction_table[inst_op] != NULL),
			"Instruction description in spim table cannot be null");
...
{
	CHECK_INVARIANT (opd1, "Opd1 cannot be NULL for a Control_Flow_IC_Stmt IC Stmt");
    //TODO check invariant depending on ic_format
	//CHECK_INVARIANT (opd2, "Opd2 cannot be NULL for a Control_Flow_IC_Stmt IC Stmt");
	//CHECK_INVARIANT (opd3, "Opd3 cannot be NULL for a Control_Flow_IC_Stmt IC Stmt");
	...
	switch (ic_format)
	{
	case i_op_o1_o2_o3: /* 3 operands e.g `bne t0 , zero : goto label3` */
			file_buffer << " " << operation_name << ":\t";
			opd1->print_ics_opd(file_buffer);
			file_buffer << " , ";
			opd2->print_ics_opd(file_buffer);
	...
	case i_op_o1: /* 1 operands e.g `goto label3` */
			file_buffer << " " << operation_name << "\t";
			file_buffer << "label";
			opd1->print_ics_opd(file_buffer);
			file_buffer << "\n";
